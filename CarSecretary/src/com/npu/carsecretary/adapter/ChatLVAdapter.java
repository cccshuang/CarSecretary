...
import com.npu.carsecretary.gif.AnimatedGifDrawable;
import com.npu.carsecretary.gif.AnimatedImageSpan;
...
public class ChatLVAdapter extends BaseAdapter {
	...
	private SpannableStringBuilder handler(final TextView gifTextView,String content) {
		...
		String regex = "(\\#\\[menu_pic/png/f_static_)\\d{3}(.png\\]\\#)";
		Pattern p = Pattern.compile(regex);
		Matcher m = p.matcher(content);
		while (m.find()) {
			String tempText = m.group();
			try {
				String num = tempText.substring("#[menu_pic/png/f_static_".length(), tempText.length()- ".png]#".length());
				String gif = "menu_pic/gif/f" + num + ".gif";
				/**
